...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] =
    sc.textFile(WikipediaData.filePath).map(WikipediaData.parse).cache()
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)(
      (curr: Int, wk: WikipediaArticle) =>
        if (wk.mentionsLanguage(lang)) 1 else curr,
      (acc: Int, curr: Int) => acc + curr
    )
  ...
  def makeIndex(
    ...
    rdd
      .flatMap(
        wk =>
          langs
            .filter(lang => wk.mentionsLanguage(lang))
            .map(lang => (lang, wk))
      )
      .groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String],
    ...
    rdd
      .flatMap(
        wk =>
          langs.filter(lang => wk.mentionsLanguage(lang)).map(lang => (lang, 1))
      )
